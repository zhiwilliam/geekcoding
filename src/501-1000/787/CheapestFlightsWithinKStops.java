class Solution {
  ...
  private void dfs(Map<Integer, List<int[]>> map, Set<Integer> visited, int src, int dst, int K, int cost) {
    if (src == dst) {
      total = cost;
      return;
    }

    if (K < 0 || visited.contains(src) || !map.containsKey(src))
      return;

    visited.add(src);

    for (int[] current : map.get(src)) {
      if (current[1] + cost > total)
        continue;
